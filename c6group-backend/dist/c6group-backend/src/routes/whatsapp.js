...
router.post('/send', async (req, res, next) => {
    try {
        ...
        if (!message) {
            throw (0, errorHandler_1.createError)('Message is required', 400, 'MISSING_MESSAGE');
        }
        ...
        if (WHATSAPP_ACCESS_TOKEN && WHATSAPP_PHONE_NUMBER_ID && phoneNumber) {
            try {
                await axios_1.default.post(`${WHATSAPP_API_URL}/${WHATSAPP_PHONE_NUMBER_ID}/messages`, {
                ...
                logger_1.logger.error('WhatsApp API send failed:', waError);
                await prisma_1.prisma.conversation.update({
                    where: { id: conversation.id },
                    data: { status: 'failed' },
                });
            ...
            for (const change of item.changes || []) {
                if (change.value?.messages) {
                    for (const message of change.value.messages) {
                        ...
                        logger_1.logger.info(`WhatsApp message received from ${phoneNumber}: ${messageBody.substring(0, 50)}`);
                        ...
                        if (autoResponse) {
                            ...
                            if (WHATSAPP_ACCESS_TOKEN && WHATSAPP_PHONE_NUMBER_ID) {
                                try {
                                    ...
                                    logger_1.logger.error('Auto-response send failed:', sendError);
                                }
                            }
                        }
                    }
 ...
 */
router.get('/templates', auth_1.authenticate, async (req, res, next) => {
    try {
        const templates = [
            {
                id: 'welcome',
