...
router.get('/dashboard', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const [aiUsage, totalAiCalls, auditResults, subscription] = await Promise.all([
            ...
            prisma_1.prisma.aIUsage.count({
                where: { userId },
            }),
            prisma_1.prisma.auditResult.findMany({
                where: { userId },
 ...
 */
router.get('/customers', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const [auditResults, totalAudits, auditStats] = await Promise.all([
            ...
            prisma_1.prisma.auditResult.count({
                where: { userId: req.user.userId },
            }),
            prisma_1.prisma.auditResult.aggregate({
                where: { userId: req.user.userId },
        ...
        const aiUsageByAgent = await prisma_1.prisma.aIUsage.groupBy({
            by: ['agentType'],
            where: { userId: req.user.userId },
            _count: { id: true },
        });
 ...
 */
router.get('/ai-usage', auth_1.authenticate, async (req, res, next) => {
    try {
 ...
 */
router.get('/admin', auth_1.authenticate, (0, auth_1.authorize)(types_1.UserRole.ADMIN), async (req, res, next) => {
    try {
        const { startDate, endDate } = req.query;
        const start = startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
        const end = endDate ? new Date(endDate) : new Date();
        const [totalUsers, newUsers, activeSubscriptions, totalAiCalls, totalAiCost, usersByPackage,] = await Promise.all([
...
// Helper function
function getTopIndustries(audits) {
    const counts = {};
