...
import { getFrontendPackage } from '@/config/commercial';
  ...
  return (
    <section className="border-y border-slate-100 bg-slate-50/50">
      <div className="container-max py-8 section-padding">
        <p className="text-center text-xs font-semibold uppercase tracking-[0.18em] text-slate-400 mb-6">Built to work with the tools South African businesses already use</p>
        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-4">{logos.map((name) => <span key={name} className="text-lg font-bold text-slate-300 select-none">{name}</span>)}</div>
      </div>
    </section>
  );
}
...
function HeroSection() {
  return (
    <section className="pt-24 pb-16 md:pt-28 md:pb-20 overflow-hidden">
      <div className="container-max section-padding"><div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div className="max-w-xl"><span className="eyebrow mb-4 inline-block">AI-Powered Business Intelligence</span><h1 className="mb-6">Understand your business. <span className="text-emerald-600">Then build the growth machine.</span></h1><p className="text-lg text-slate-600 mb-8 leading-relaxed">C6GROUP audits your business, identifies practical opportunities and connects you with AI-powered tools and growth systems designed for South African businesses.</p><div className="flex flex-col sm:flex-row gap-3 mb-8"><Link to="/audit" className="btn-primary">Free AI Business Audit <ArrowRight className="ml-2 w-4 h-4" /></Link><Link to="/ai-tools" className="btn-secondary">Explore AI Tools</Link></div><div className="flex flex-wrap items-center gap-5 text-sm text-slate-500"><span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-emerald-500" /> No credit card required</span><span className="flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-emerald-500" /> Start with an audit</span></div></div>
        <div className="relative"><img src="/hero-main.jpg" alt="South African business owner using C6GROUP business tools" className="rounded-2xl shadow-xl w-full object-cover aspect-[4/3]" /><div className="absolute -bottom-6 -left-6 bg-white rounded-xl shadow-lg border border-slate-100 p-4 hidden md:block"><div className="flex items-center gap-3"><div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center"><TrendingUp className="w-5 h-5 text-emerald-600" /></div><div><p className="text-sm font-bold text-slate-900">Business Audit</p><p className="text-xs text-slate-500">Find your biggest opportunities</p></div></div></div></div>
      </div></div>
    </section>
  );
}
